...
byte_array_t cp_e2ap_plmn_to_ba(e2ap_plmn_t const* src)
{
  ...
  // MCC
  uint8_t const mcc_hundred = src->mcc / 100;
  uint8_t const mcc_ten = (src->mcc / 10) % 10;
  uint8_t const mcc_unit = src->mcc % 10;
  assert(mcc_hundred < 10 && mcc_ten < 10 && mcc_unit < 10);
  
  // MNC
  assert(src->mnc_digit_len == 3 || src->mnc_digit_len == 2);
  uint8_t const mnc_hundred = src->mnc_digit_len == 3 ? (src->mnc / 100) : 15; //0xF;
  uint8_t const mnc_ten = (src->mnc / 10) % 10;
  uint8_t const mnc_unit = src->mnc % 10;
  assert(mnc_ten < 10 && mnc_unit < 10);

  byte_array_t ba = {.len = 8};
  ba.buf = calloc(ba.len, sizeof(uint8_t));
  assert(ba.buf != NULL && "Memory exhausted");

  // Each BCD e.g., mcc_hundred has 4 bits
  ba.buf[0] = (mcc_ten << 4) | mcc_hundred; // Octet 1: [mcc_ten|mcc_hundred]
  ba.buf[1] = (mnc_hundred << 4) | mcc_unit; // Octet 2: [mnc_hundred|mcc_unit]
  ba.buf[2] = (mnc_unit << 4) | mnc_ten; // Octet 3: [mnc_unit|mnc_ten]
...
void gen_rrm_policy_ratio_list(seq_ran_param_t* RRM_Policy_Ratio_List)
{
  ...
  int max_ratio[] = {20, 50, 100};
  int min_ratio[] = {20, 50, 0};
  int dedicated_ratio[] = {20, 50, 0};
