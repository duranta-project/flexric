...
static
byte_array_t str_to_ba(const char* str)
{
  assert(str != NULL);
  size_t const sz = strlen(str);
...
void test_setup_request()
{
  ...
  assert(ran_func_item != NULL && "Memory exhausted");
  ...
  e2ap_node_component_config_add_t* comp_conf_add = calloc(1, sizeof(e2ap_node_component_config_add_t));
  assert(comp_conf_add != NULL && "Memory exhausted");
...
comp_conf_add[0].e2_node_comp_id.type =  NG_E2AP_NODE_COMP_INTERFACE_TYPE;
  ...
  // Mandatory
  // 9.2.27
  const char str_req[] = "NG SETUP REQUEST, 3GPP 38.413 [19] clause 9.2.6.1 ";
  const char str_res[] = "NG SETUP RESPONSE, 3GPP 38.413 [19] clause 9.2.6.2";
  comp_conf_add[0].e2_node_comp_conf.request = str_to_ba( str_req );
  comp_conf_add[0].e2_node_comp_conf.response = str_to_ba( str_res );
...
int main()
{
    ...
    // ToDO: New in v2
    // test_removal_request(); 
    // test_removal_response();
