...
- Email subject: [SEC-VUL-DURANTA-OAI-FLEXRIC]: "Mention the affected functionality
  here".
...
repository](https://github.com/duranta-project/flexric/issues). In
this case, please follow the format below to open the issue:

- Issue title: [AI-SEC-VUL-DURANTA-OAI-FLEXRIC]: "Mention the affected functionality
  here".
- Commit: it should be the latest commit, or a very recent one, as of writing
  the bug report.
- Issue description (please do not write long paragraphs).
...
- AI tools are good at fixing vulnerabilities; you can ask your AI tool to
  provide a patch.
- Add the https://github.com/duranta-project/flexric/labels/security
  label to the issue.

### Scope

The `duranta-project/flexric` repository contains the FlexRIC nearRT-RIC, the
E2 agent library used by E2 nodes, the xApp SDK (C and Python bindings), the
O-RAN and custom service models (KPM, RC, MAC, RLC, PDCP, GTP, slice, TC), E2
node emulators, deployment artifacts, and CI/release tooling.
...
- Memory corruption, crashes, or denial of service in the nearRT-RIC, the E2
  agent, or an xApp triggered by malformed E2AP or E42 messages (any supported
  E2AP version) received over SCTP.
- Memory corruption, crashes, or denial of service triggered by malformed
  service model payloads (ASN.1, FlatBuffers, or plain encoding), including
  RAN function definitions, indications, and control messages.
- Unauthorized E2 node or xApp connections, subscriptions, or control
  requests, or state-machine bypasses in E2 setup, subscription, indication,
  and control procedures.
- Leakage of UE identifiers, RAN metrics, or other sensitive data through the
  RIC, xApps, logs, or the xApp database.
- Remote code execution, privilege escalation, arbitrary file access, or command
  execution through configuration parsing, service model plugin loading,
  protocol handling, logging, tools, or deployment artifacts.
- Container, OpenShift, Docker, or default-configuration issues that expose
  sensitive services, secrets, or unsafe privileges in documented deployments.
...
- Issues coming from third-party projects used for Duranta/OAI testing, such as
  the RAN, core network, or other nearRT-RICs FlexRIC is integrated with.
- Denial of service in experimental, test-only, or emulator-only code (for
  example the E2 node emulators and example xApps) unless it demonstrates a
  realistic impact on supported deployments.
