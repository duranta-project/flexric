...
void free_label_info(label_info_lst_t *l) 
{
    ...
    if (l->noLabel) free(l->noLabel);
    if (l->plmn_id) free(l->plmn_id);
    if (l->sliceID != NULL) {
        free_s_nssai_e2sm(l->sliceID);
        free(l->sliceID);
    }
    if (l->fiveQI != NULL) free(l->fiveQI);

    if (l->qFI != NULL) { assert(false && "not implemented"); }
    if (l->qCI != NULL) { assert(false && "not implemented"); }
    if (l->qCImax != NULL) { assert(false && "not implemented"); }
    if (l->qCImin != NULL) { assert(false && "not implemented"); }
    if (l->aRPmax != NULL) { assert(false && "not implemented"); }
    if (l->aRPmin != NULL) { assert(false && "not implemented"); }
    if (l->bitrateRange != NULL) { assert(false && "not implemented"); }
    if (l->layerMU_MIMO != NULL) { assert(false && "not implemented"); }
    if (l->sUM != NULL) { assert(false && "not implemented"); }

    if (l->distBinX != NULL) free(l->distBinX);
    if (l->distBinY != NULL) free(l->distBinY);
    if (l->distBinZ != NULL) free(l->distBinZ);
    
    if (l->preLabelOverride != NULL) { assert(false && "not implemented"); }
    if (l->startEndInd != NULL) { assert(false && "not implemented"); }
    if (l->min != NULL) { assert(false && "not implemented"); }
    if (l->max != NULL) { assert(false && "not implemented"); }
    if (l->avg != NULL) {
        // This was missing a free call
        free(l->avg);
    }
}
...
bool eq_label_info(const label_info_lst_t *l1, const label_info_lst_t *l2)
{
    ...
    if(l1->noLabel != NULL || l2->noLabel != NULL){
        if(l1->noLabel == NULL || l2->noLabel == NULL) return false;
        if(*l1->noLabel != *l2->noLabel) return false;
    }  

    if (eq_e2sm_plmn(l1->plmn_id, l2->plmn_id) != true) return false;
    if (eq_s_nssai_e2sm(l1->sliceID, l2->sliceID) != true) return false;

    if(l1->fiveQI != NULL || l2->fiveQI != NULL){
        if(l1->fiveQI == NULL || l2->fiveQI == NULL) return false;
        if(*l1->fiveQI != *l2->fiveQI) return false;
    } 
    ...
    if(l1->avg != NULL || l2->avg != NULL){
        if(l1->avg == NULL || l2->avg == NULL) return false;
        if(*l1->avg != *l2->avg) return false;
    }
