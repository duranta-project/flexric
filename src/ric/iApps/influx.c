...
void notify_influx_listener(sm_ag_if_rd_ind_t const* data)
{
  ...
  assert(data->type == MAC_STATS_V0 || data->type == RLC_STATS_V0 || data->type == PDCP_STATS_V0 
      ...
      || data->type == TC_STATS_V0 || data->type == RAN_CTRL_STATS_V1_03|| data->type == SRS_STATS_V0);
...
//  printf("Influx db data called!!!\n");
  ...
  if(data->type == MAC_STATS_V0){
   ...
   // }
    ...
    srs_ind_msg_t const* srs = &data->srs.msg;

    for(uint32_t i = 0; i < srs->len; ++i){
      char stats[512] = {0};
      srs_indication_stats_impl_t* indication_stats = &srs->indication_stats[i];

      to_string_srs_indication(indication_stats, srs->tstamp, stats, 512);
      int const rc = sendto(sockfd, stats, strlen(stats),  MSG_CONFIRM, (const struct sockaddr *) &servaddr, sizeof(servaddr));
      assert(rc != -1);
    }
  } else {
